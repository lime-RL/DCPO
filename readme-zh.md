...
在之前的研究中，例如 GRPO 和 DAPO，第i步step中，第j个response 中标记 t 的优势 $\hat{A}_{j,t}^i$ 是通过将奖励 $R^i_j$  通过第 i 步生成的 G 个response的奖励均值 $\mu^i$ 和标准差 $\mu^i$ 来计算的。当同一提示的奖励相同->优势为零-> loss 为0，这些response不会对模型更新做出贡献，从而造成response浪费。
...
正如我们所看到的，当概率较小时，我们动态自适应裁剪方法的q(x) 值比固定方法（无论是非对称固定方法还是对称固定方法）要大得多，但又在可控的范围内。
