...
__all__ = ["register_adv_est", "get_adv_estimator_fn"]
...
from verl.trainer.ppo.core_name_space import AdvantageEstimator, LossCalculator
...
def get_mean_std(old_mean, old_std, times, new_mean, new_std):
    old_times = times - 1
    ...
    if old_times == 0:
        ...
        mean = (old_mean * old_times + new_mean) / (old_times + 1)
        # var = old_times * (old_std**2) + (new_std**2) + old_times / (old_times) * (new_mean - old_mean) ** 2
        var = old_times * (old_std**2) + (new_std**2) + old_times / (times) * (new_mean - old_mean) ** 2
        var = var / times
        std = np.sqrt(var)
    print(f"{mean=}, {std=}, {type(mean)=}, {type(std)=}")
    return mean, std


@register_adv_est(AdvantageEstimator.DCPO)  # or simply: @register_adv_est("grpo_passk")
def compute_dcpo_outcome_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: torch.Tensor,
    old_times,
    ...
    with torch.no_grad():
        ...
        for i in range(bsz):
            id2score[index[i]].append(scores[i])
            id2mean[index[i]] = old_mean[i]
            id2std[index[i]] = old_std[i]
            id2times[index[i]] = old_times[i]
        ...
        for idx in id2score:
            ...
            if len(id2score[idx]) > 1:
                ...
                new_mean = torch.mean(torch.tensor(id2score[idx]))
                new_std = torch.std(torch.tensor(id2score[idx]))
                id2mean_new[idx] = new_mean
                id2std_new[idx] = new_std
                id2mean[idx], id2std[idx] = get_mean_std(id2mean[idx] * user_old_mean_ratio, id2std[idx] * user_old_mean_ratio, id2times[idx] + 1, new_mean, new_std)
            else:
                raise ValueError(f"no score in prompt index: {idx}")
        ...
        for i in range(bsz):
            ...
            # scores[i] = torch.where(scores[i] > 0, abs(scoresi), -abs(scoresi))
            scores[i] = torch.where(abs(scoresi1) < abs(scoresi2), scoresi1, scoresi2)
        # if user_length_ratio:
...
def agg_loss(loss_mat: torch.Tensor, loss_mask: torch.Tensor, loss_agg_mode: str, batch_response_len: float = None, batch_size: float = None):
    """
    ...
    Args:
        ...
        batch_response_len: `float`:
             ensure compatibility with different micro batch sizes for token mean.
        batch_size `float`:
             ensure compatibility with different micro batch sizes.
    Returns:
        loss: `a scalar torch.Tensor`
            aggregated loss
    """

    if loss_agg_mode == LossCalculator.TM:
        if not batch_response_len:
        ...
        if not batch_size:
            ...
            seq_losses = torch.sum(loss_mat * loss_mask, dim=-1)  # token-sum
            loss = torch.sum(seq_losses) / max(batch_size, 1)  # seq-mean
    elif loss_agg_mode == LossCalculator.SMTM:
        if not batch_size:
            ...
            seq_losses = torch.sum(loss_mat * loss_mask, dim=-1) / torch.sum(loss_mask, dim=-1)  # token-mean
            loss = torch.sum(seq_losses) / max(batch_size, 1)  # seq-mean
    elif loss_agg_mode == LossCalculator.SMTSN:
...
def get_clip_low_high(ref_logprob, low_clip=0.1, high_clip=0.2, clip_ratio_c=float("inf")):
...
@register_policy_loss("dcpo")
def compute_policy_loss_dcpo(
    old_log_prob: torch.Tensor,
    log_prob: torch.Tensor,
    advantages: torch.Tensor,
    response_mask: torch.Tensor,
    loss_agg_mode: str = "token-mean",
    config: Optional[DictConfig | AlgoConfig] = None,
    batch_response_len: Optional[torch.Tensor | float] = None,
    batch_size: Optional[torch.Tensor | float] = None,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    """
    Compute the clipped policy objective and related metrics for PPO.
    ...
    clip_type = config.get("clip_type", "origin")
    ...
    if cliprange_high is None:
        cliprange_high = cliprange
    if clip_type == "dynamic":  # grpo
        cliprange_low, cliprange_high = get_clip_low_high(old_log_prob, cliprange_low, cliprange_high, clip_ratio_c)
        assert clip_ratio_c > 1.0, "The lower bound of the clip_ratio_c for dual-clip PPO should be greater than 1.0," + f" but get the value: {clip_ratio_c}."
    elif clip_type == "dual":  # dapo dcpo
        assert clip_ratio_c > 1.0, "The lower bound of the clip_ratio_c for dual-clip PPO should be greater than 1.0," + f" but get the value: {clip_ratio_c}."
        cliprange_low, cliprange_high = 1 - cliprange_low, 1 + cliprange_high
    elif clip_type == "sequence":
        seq_level_log_weight = verl_F.masked_mean(negative_approx_kl, response_mask).unsqueeze(-1)
        ratio = torch.exp(seq_level_log_weight)
        pg_losses1 = -advantages * ratio
        pg_losses2 = -advantages * torch.clamp(ratio, 1 - cliprange_low, 1 + cliprange_high)
        pg_loss = torch.maximum(pg_losses1, pg_losses2)
        pg_clipfrac = verl_F.masked_mean(torch.gt(pg_losses2, pg_losses1).float(), response_mask)
        return pg_loss, pg_clipfrac, ppo_kl, torch.tensor(0.0, requires_grad=True, device=pg_loss.device)

    elif clip_type == "sequence-token":
        seq_level_log_weight = verl_F.masked_mean(negative_approx_kl, response_mask)
        seq_level_log_weight = seq_level_log_weight.detach().unsqueeze(-1)  # Stop gradient
        log_importance_weights = log_prob - log_prob.detach() + seq_level_log_weight
    ...
    pg_losses2 = -advantages * torch.clamp(ratio, cliprange_low, cliprange_high)  # - clip(ratio, 1-cliprange, 1+cliprange) * A
    clip_pg_losses1 = torch.maximum(pg_losses1, pg_losses2)  # max(-ratio * A, -clip(ratio, 1-cliprange, 1+cliprange) * A)
    pg_clipfrac = verl_F.masked_mean(torch.gt(pg_losses2, pg_losses1).float(), response_mask)

    pg_losses3 = -advantages * clip_ratio_c
    # dual clip or dynamic clip
    if clip_type in ["dynamic", "dual"]:

        clip_pg_losses2 = torch.min(pg_losses3, clip_pg_losses1)
        pg_clipfrac_lower = verl_F.masked_mean(torch.gt(clip_pg_losses1, pg_losses3) * (advantages < 0).float(), response_mask)

        pg_losses = torch.where(advantages < 0, clip_pg_losses2, clip_pg_losses1)
        pg_loss = agg_loss(loss_mat=pg_losses, loss_mask=response_mask, loss_agg_mode=loss_agg_mode, batch_response_len=batch_response_len, batch_size=batch_size)

        return pg_loss, pg_clipfrac, ppo_kl, pg_clipfrac_lower
    else:
        # origin clip
        pg_loss = agg_loss(loss_mat=clip_pg_losses1, loss_mask=response_mask, loss_agg_mode=loss_agg_mode, batch_response_len=batch_response_len, batch_size=batch_size)
